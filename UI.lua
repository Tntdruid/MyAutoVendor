-- UI.lua – MyAutoVendor – Clean Dark Panel + Right-click Remove + Global Sell Tab
local ADDON_NAME = ...
...
local function CreateMainFrame()
    ...
    local tabGlobal = MakeTab(264, "Global Sell")
    ...
    drop:SetScript("OnEnter", function()
        drop.bg:SetVertexColor(0.18, 0.18, 0.18, 0.9)
    end)
    ...
    drop:SetScript("OnReceiveDrag", function()
        ...
        if addon.activeTab == "global" then
            addon.profile.globalSellList[itemID] = { ts = time(), link = "item:"..itemID }
        else
        ...
        addon:RefreshUI()
        ClearCursor()
        print("Tilføjet: " .. (GetItemInfo(itemID) or ("item:"..itemID)))
    end)

    ---------------------------------------------------------------------
    -- BUTTONS (Options / Undo / Clear)
    ---------------------------------------------------------------------
    ...
    function addon:RefreshUI()
        ...
        local list =
            ...
            self.activeTab == "global" and global
        ...
        tabGlobal:SetActive(self.activeTab == "global")
    end
    ...
    tabGlobal:SetScript("OnClick", function()
        addon.activeTab = "global"
        addon:RefreshUI()
    end)
...
-- UI.lua – MyAutoVendor (AzerothCore + Ace3) – Clean Dark Panel + Right-click Remove
local ADDON_NAME = ...
local addon = _G[ADDON_NAME]
...
local function CreateMainFrame()
    ...
    ---------------------------------------------------------------------
    -- SCROLL LIST (Right-click remove)
    ---------------------------------------------------------------------
    local scroll = CreateFrame("ScrollFrame", "MyAutoVendorScroll", frame, "UIPanelScrollFrameTemplate")
    scroll:SetPoint("TOPLEFT", 16, -72)
    scroll:SetPoint("BOTTOMRIGHT", -40, 200)
    ...
    for i = 1, MAX_ROWS do
        ...
        row:SetScript("OnClick", function(self, button)
            if button == "RightButton" then
                if self.itemID then
                    safe(addon.RemoveItem, addon, self.itemID)
                end
                return
            end
    ...
    ---------------------------------------------------------------------
    -- INPUT + BUTTONS (Add/Remove fjernet)
    ---------------------------------------------------------------------
